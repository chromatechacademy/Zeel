...
public class BookRoomStepDef {
    ...
    public void a_user_scrolls_down_to_the_book_this_room_section() throws InterruptedException {
        JavascriptUtils.scrollIntoView(mainPageContactForm.bookThisRoomButton);
        Thread.sleep(3000);
    }

    @When("the user clicks on Book this room button")
    public void the_user_clicks_on_Book_this_room_button() throws InterruptedException {
    ...
    public void the_user_selects_single_night() throws InterruptedException {
        Actions actions = new Actions(WebDriverUtils.driver);
        actions.dragAndDrop(mainPageBookForm.firstDay, mainPageBookForm.lastDay).perform();
        Thread.sleep(3000);
    }

    @When("the user fills out the following fields {string}, {string}, {string}, {string}")
    public void the_ser_fills_out_the_following_fields(String firstName, String lastName, String email, String phone) throws InterruptedException {
    ...
    public void the_user_clicks_on_Book_button() throws InterruptedException {
        CommonUtils.click(mainPageBookForm.bookButton);
        Thread.sleep(6000);
    }

    @Then("the successful message is displayed on the confirmation modal")
    public void the_successful_message_is_displayed_on_the_confirmation_modal() {

    }

    @Then("the message is displayed on the confirmation modal {string}, {string}, {string}")
    public void the_message_is_displayed_on_the_confirmation_modal(String string, String string2, String string3) {
